...
describe Leo::V1::HealthRecords do
  ...
  before { allow_any_instance_of(SyncService).to receive(:sync_vitals).and_return(true) }
  before { allow_any_instance_of(SyncService).to receive(:sync_allergies).and_return(true) }
  before { allow_any_instance_of(SyncService).to receive(:sync_vaccines).and_return(true) }
  before { allow_any_instance_of(SyncService).to receive(:sync_medications).and_return(true) }
