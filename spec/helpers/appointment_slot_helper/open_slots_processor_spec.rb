...
RSpec.describe AppointmentSlotsHelper, type: :helper do
  describe "OpenSlotsProcessor" do
    ...
    let(:provider_profile) { provider.create_provider_profile!(athena_id: 2, athena_department_id: 2, provider_id: provider.id) }
    ...
    let!(:schedule){create( :provider_schedule,
                            athena_provider_id: provider_profile.athena_id,
    ...
    describe "get_provider_availability" do
      ...
      it "computes availability" do
        expect(osp.get_provider_availability(athena_provider_id: provider_profile.athena_id, date: date)).to eq(provider_availability)
      end
    end
    ...
    describe "get_open_slots" do
      ...
      it "returns slots" do
        expect(osp.get_open_slots(athena_provider_id: provider_profile.athena_id, date: date, durations: [ 60 ])).to eq(open_slots)
      end
    end
  end
end
