module AppointmentSlotsHelper
  class OpenSlotsProcessor
    ...
    def get_provider_availability(athena_provider_id:, date:)
      ...
      provider = Provider.find_by!(athena_id: athena_provider_id)
      Appointment.where(provider: provider).where("start_datetime >= ? AND start_datetime <= ?", start_datetime, end_datetime).where(appointment_status_id: booked_statuses).find_each {
