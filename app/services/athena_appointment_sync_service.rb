class AthenaAppointmentSyncService < AthenaSyncService
  def post_appointment(leo_appt)
    if leo_appt.athena_id == 0
      ...
      slots = Slot.free.where(provider_sync_profile: leo_appt.provider.provider_sync_profile).start_datetime_between(leo_appt.start_datetime, leo_appt.end_datetime).order("start_datetime")
